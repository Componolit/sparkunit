-- $Id: spark_io.adb 15664 2010-01-19 12:16:03Z rod chapman $
--------------------------------------------------------------------------------
...
is
  ...
  procedure Dispose is new Unchecked_Deallocation (Ada.Text_IO.File_Type, File_PTR);
...
-- Character IO
  ...
  begin
     if Is_In( File) then
        Ada.Text_IO.Get_Immediate( File_Ref( File), Item);
        Status := Ok;
     else
        Item := Character'First;
        Status := Mode_Error;
